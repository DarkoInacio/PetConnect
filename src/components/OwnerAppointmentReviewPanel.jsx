...
export function OwnerAppointmentReviewPanel({ appointmentId, providerName, onReviewSaved }) {
	...
	const load = useCallback(
		async (signal) => {
			...
			try {
				...
				setElig(e);
				if (e?.review) {
					setForm({ rating: e.review.rating, comment: e.review.comment || '' });
				} else {
	...
	async function onSubmit(e) {
		...
		try {
			...
			setElig(e2);
			if (e2?.review) {
				setForm({ rating: e2.review.rating, comment: e2.review.comment || '' });
			}
			onReviewSaved?.();
		} catch (err) {
				...
				<form className="review-form" onSubmit={onSubmit}>
					...
					<label className="review-field">
						...
						<textarea
							...
							rows={3}
							maxLength={2000}
							placeholder="Cómo fue la atención, tiempos de espera, etc."
						/>
					</label>
					{elig.hasReview ? (
